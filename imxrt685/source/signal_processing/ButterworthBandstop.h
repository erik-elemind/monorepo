...
#ifndef _BUTTERWORTH_BANDSTOP_H_
#define _BUTTERWORTH_BANDSTOP_H_
...
class ButterworthBandstop{
  ...
  public:
    ButterworthBandstop(int order, double lowFreq, double highFreq, double sampleFreq){
      ButterworthBandstop();
      design(order,lowFreq,highFreq,sampleFreq);
    }
    ButterworthBandstop(): index_(0), numcof_(0){
    }
    ~ButterworthBandstop(){
    }
    void reset(){
    ...
    void design(int order, double lowFreq, double highFreq, double sampleFreq, bool resetCache){
      ...
      dcof_bwbs( order, f1f, f2f, dbwbp_rcof_, dbwbp_tcof_, dbwbp_dcof_ );
      ccof_bwbs( order, f1f, f2f, cbwbp_ccof_ );
      double sf = sf_bwbs( order, f1f, f2f );
...
class ButterworthBandstopFreq : public ButterworthBandstop<T,MAX_BB_ORDER_T>{
  protected:
  ...
  public:
    ButterworthBandstopFreq() : ButterworthBandstop<T,MAX_BB_ORDER_T>(){
    }
    ~ButterworthBandstopFreq(){
    }
...
#endif /* _BUTTERWORTH_BANDSTOP_H_ */
