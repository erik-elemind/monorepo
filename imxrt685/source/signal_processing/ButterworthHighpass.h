...
#ifndef _BUTTERWORTH_HIGHPASS_H_
#define _BUTTERWORTH_HIGHPASS_H_
...
class ButterworthHighpass{
  protected:
    ...
    double dbwhp_rcof_[2*MAX_BB_ORDER_T];
    double dbwhp_dcof_[4*MAX_BB_ORDER_T];
    double cbwhp_ccof_[2*MAX_BB_ORDER_T+1];
  public:
    ButterworthHighpass(int order, double cutFreq, double sampleFreq){
      ButterworthHighpass();
      design(order,cutFreq,sampleFreq);
    }
    ButterworthHighpass(): index_(0), numcof_(0){
    }
    ~ButterworthHighpass(){
    }
    void reset(){
    ...
    //int *ccof_bwhp( int n, int* ccof  );
    //double sf_bwhp( int n, double fcf );

    void design(int order, double cutFreq, double sampleFreq, bool resetCache){
      double fcf = 2*cutFreq/sampleFreq;
      // compute coefficients and scaling factor
      dcof_bwhp( order, fcf, dbwhp_rcof_, dbwhp_dcof_ );
      ccof_bwhp( order, cbwhp_ccof_);
      double sf = sf_bwhp( order, fcf );
      ...
      for(size_t i=0; i<numcof_; i++){
        dcof_[i] = dbwhp_dcof_[i];
        ccof_[i] = cbwhp_ccof_[i]*sf;
      }
    }
...
class ButterworthHighpassFreq : public ButterworthHighpass<T,MAX_BB_ORDER_T>{
  protected:
  ...
  public:
    ButterworthHighpassFreq() : ButterworthHighpass<T,MAX_BB_ORDER_T>(){
    }
    ~ButterworthHighpassFreq(){
    }
...
#endif /* _BUTTERWORTH_HIGHPASS_H_ */
